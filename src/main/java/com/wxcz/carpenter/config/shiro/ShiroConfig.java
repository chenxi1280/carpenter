...
import com.wxcz.carpenter.config.shiro.filter.ShiroAuthFilter;
...
public class ShiroConfig {
    ...
    public RememberMeManager rememberMeManager() { // 5
        ...
        SimpleCookie cookie = new SimpleCookie("wxcz-RememberMe");
...
//        sessionValidationScheduler.setInterval(30 * 60 * 1000);// 设置循环检测的时间
    ...
    public DefaultWebSessionManager defaultWebSessionManager(SessionDAO sessionDAO) { // 6
        ...
        SimpleCookie sessionIdCookie = new SimpleCookie("wxcz-java-session-id");//设置一个SessionCookie
    ...
    public LogoutFilter getLogoutFilter() { // 在ShiroFilterFactoryBean中使用
        ...
        logoutFilter.setRedirectUrl("/back/logoutPage");    //  自定义退出登录之后，跳转访问的方法路径
        return logoutFilter;
    }
